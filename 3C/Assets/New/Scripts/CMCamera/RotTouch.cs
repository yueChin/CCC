...
public class RotTouch : MonoBehaviour
{
    ...
    {   
        ...
        Vector3 left = FollowTgt.transform.position + FollowTgt.transform.right * -5;
        Vector3 right = FollowTgt.transform.position + FollowTgt.transform.right * 5;

        float disL = Vector3.Distance(Camera.main.transform.position,left);
        float disR = Vector3.Distance(Camera.main.transform.position, right);
    ...
    {   
        ...
        if (testFacvl < 180)
        {   
            CinemachineFreeLook.m_XAxis.m_InputAxisValue = 30 * m_FwdRotMul;
            if (testFacvl < 60)
            {   
                CinemachineFreeLook.m_XAxis.m_InputAxisValue = 0.2f * m_FwdRotMul;
            }   
            if (testFacvl < 20)
            {   
                EndStartUpdateFwdTurn();
