...
namespace CapsuleCharacterCollisionDetection
{
	...
	public static class ConvexHull
	{
		public static void GenerateHull(IList<Vector3> points, bool splitVerts, ref List<Vector3> vertsResults, ref List<int> trisResults, ref List<Vector3> normalsResults)
		{
			...
			ConvexHullCreationResult<DefaultVertex, DefaultConvexFace<DefaultVertex>> hull = MIConvexHull.ConvexHull.Create(verts, .000001f); //We need to use a higher tolerance than the Constants.DefaultPlaneDistanceTolerance, otherwise we get collinear results which causes vector3.zero normals which causes issues.
			if(hull.Outcome == ConvexHullCreationResultOutcome.Success)
			{
				...
				int index = 0;
				foreach(DefaultConvexFace<DefaultVertex> face in hull.Result.Faces)
				{
					double[] pos = face.Vertices[0].Position;
	...
	public class ConvexHullGenerator
	{
		...
		//Seems to make a good difference on things like flat plane mesh.
			...
			int b0 = initialSimplex[0];
			int b1 = initialSimplex[1];
			int b2 = initialSimplex[2];
			int b3 = initialSimplex[3];
			...
			// TODO use epsilon?
			bool above = Dot(v3 - v1, Cross(v1 - v0, v2 - v0)) > EPSILON;
		...
		void FindHorizon(IList<Vector3> points, Vector3 point, int fi, Face face)
		{
			...
			// current triangle.
			{
				Face oppositeFace = faces[face.Opposite0];
			...
			{
				Face oppositeFace = faces[face.Opposite1];
			...
			{
				Face oppositeFace = faces[face.Opposite2];
		...
		void SearchHorizon(IList<Vector3> points, Vector3 point, int prevFaceIndex, int faceCount, Face face)
		{
			...
			{
				Face oppositeFace = faces[nextFaceIndex0];
			...
			{
				Face oppositeFace = faces[nextFaceIndex1];
		...
		void ConstructCone(IList<Vector3> points, int farthestPoint)
		{
			...
			for (int i = 0; i < horizon.Count; i++)
			{
				...
				Face horizonFace = faces[horizon[i].Face];
